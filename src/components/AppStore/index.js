...
class AppStroe extends Component {
  state = {textHigh: tabsList[0].tabId, search: ''}
  searchInput = event => {
    this.setState({search: event.target.value})
  }
  displayCategory = tabId => {
    this.setState({textHigh: tabId})
  }
  render() {
    const {textHigh, search} = this.state
    const filteredappList = appsList.filter(
      eachOfApp =>
        eachOfApp.category.toLocaleLowerCase() === textHigh.toLocaleLowerCase(),
    )
    const searchFilterApp = filteredappList.filter(eachOF =>
      eachOF.appName.toLocaleLowerCase().includes(search.toLocaleLowerCase()),
    )
    return (
      <div className="bg-container">
        <div className="app-box-container">
          ...
          <ul className="un-orderd-app-list">
            {searchFilterApp.map(eachApp => (
